...
const Login = () => {
  ...
  const handleGoogleSuccess = async (tokenResponse) => {
    setLoading(true);
    // Google credential token is returned under access_token or credential
    // In React Google Login (implicit flow), we get an access_token. 
    // We pass it to backend google authentication endpoint
        ...
        <div className="glass-panel border p-8 rounded-3xl shadow-xl flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              disabled={loading}
              className="flex items-center justify-center gap-2 py-3 rounded-xl border border-slate-200 dark:border-dark-800 hover:bg-slate-100 dark:hover:bg-dark-800 transition-colors font-semibold text-xs text-slate-700 dark:text-slate-300"
            >
